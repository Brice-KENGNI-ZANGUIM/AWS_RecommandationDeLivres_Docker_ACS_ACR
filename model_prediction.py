...
def prediction(models, data, articles_metadata, article_embedding_80, user_id, n) :
    ...
    for _model in models :
        if _model == "Content-based Recommandation" :
            ...
            recommandations.append( 
                content_base_recommandation_par_utilisateur( user_id , _user_rating_categ , _articles_metadata, n, old_user_articles 
                                    ).reset_index(drop=True)[["article_id"]] )

        elif _model == "Collaborative Filtering - SVD" :
            ###########################################################################
            ...
            #########    Génération des recommandation d'article et ajout à la liste des recommandations      #########
            ###########################################################################################################
            recommandations.append( svd.recommend( user_id = user_id, n = n, anciens_articles=old_user_articles).reset_index(drop=True)[["article_id"]] )

        elif _model == "Embedding - Distance Similarité" :
        ...
        elif _model == "Embedding - Cosinus Similarité" :
